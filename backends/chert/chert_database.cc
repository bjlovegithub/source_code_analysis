                                     ...
                                     const Xapian::Document & document)
{
    ...
    try {
        ...
        LOGLINE(DB, "Add to record table");
        ...
        {
            LOGLINE(DB, "Check all terms in document: " << document);
            ...
            for ( ; term != term_end; ++term) {
                ...
                if (term.positionlist_begin() != term.positionlist_end()) {
                    LOGLINE(DB, "Insert into position list");
    ...
    if (++change_count >= flush_threshold) {
        LOGLINE(DB, "Flush and apply db change");
