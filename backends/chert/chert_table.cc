...
ChertTable::write_block(uint4 n, const byte * p) const
{
    ...
    LOGLINE(DB, "Write block n in the DB file from address p");
    Assert(writable);
    ...
    if (both_bases) {
        ...
        LOGLINE(DB, "delete the old base file");
    ...
    LOGLINE(DB, "Call chert_io_write");
...
ChertTable::find(Cursor * C_) const
{
    ...
    int c;
    Key key = kt.key();
    //LOGLINE(DB, "Key is: " << key);
    LOGLINE(DB, "Try to traverse the whole tree to find the key");
...
ChertTable::enter_key(int j, Key prevkey, Key newkey)
{
    ...
    LOGLINE(DB, "enter_key(j, prevkey, newkey) is called after a block split.");
    //LOGLINE(DB, "prevkey: " << prevkey << ", newkey: " << newkey);
    Assert(writable);
...
ChertTable::add_item_to_block(byte * p, Item_wr kt_, int c)
{
    ...
    LOGLINE(DB, "Add item to block at p");
    Assert(writable);
    int dir_end = DIR_END(p);
    int kt_len = kt_.size();
    ...
    LOGLINE(DB, "Dir end: " << dir_end << ", kt_len: " << kt_len
            << ", needed: " << needed);
    ...
    LOGLINE(DB, "Block p after setting: " << p);
}
...
ChertTable::add_item(Item_wr kt_, int j)
{
    ...
    LOGLINE(DB, "Add item to block at cursor level C[j]");
    Assert(writable);
    byte * p = C[j].p;
    int c = C[j].c;
    uint4 n;
    ...
    LOGLINE(DB, "Total free: " << TOTAL_FREE(p) << ", needed: " << needed);
    if (TOTAL_FREE(p) < needed) {
        LOGLINE(DB, "Free space is not enough, ready to split current tree node");
        int m;
        ...
        uint4 split_n = C[j].n;
        LOGLINE(DB, "Find next free block call next_free_block");
        C[j].n = base.next_free_block();

        LOGLINE(DB, "Before split, p content is: " << p);
        ...
        LOGLINE(DB, "After split, p is: " << p << ", and new p is: " << split_p);
        LOGLINE(DB, "Write new node to...");
        ...
        LOGLINE(DB, "Just add key to block");
...
ChertTable::add_kt(bool found)
{
    ...
    LOGLINE(DB, "Try to find key-tag in BTree and add or replace");
    Assert(writable);
    int components = 0;
