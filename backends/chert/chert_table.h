...
//	set_block_given_by(n);
    ...
    void form_key(const std::string & key_) {
        LOGLINE(DB, "Call form_key, key is: " << key_);
        ...
        LOGLINE(DB, "set key length");
        set_key_len(key_len + K1 + C2);
        LOGLINE(DB, "block p before memmove: " << p);
        std::memmove(p + I2 + K1, key_.data(), key_len);
        LOGLINE(DB, "block p after memmove: " << p);
        set_component_of(1);
    }
