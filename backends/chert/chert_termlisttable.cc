                                 ...
                                 chert_doclen_t doclen)
{
    ...
    LOGLINE(DB, "Tag after pack_uint is: " << tag);
    ...
    if (t != doc.termlist_end()) {
        LOGLINE(DB, "Term is: " << *t);
        pack_uint(tag, termlist_size);
        LOGLINE(DB, "pack_uint, termlist_size: " << termlist_size
                << ", tag: " << tag);
        string prev_term = *t;

        tag += prev_term.size();
        tag += prev_term;
        LOGLINE(DB, "Tag after adding pre_term: " << tag);
        pack_uint(tag, t.get_wdf());
        LOGLINE(DB, "Tag after adding tag: " << tag);
        --termlist_size;
        ...
        while (++t != doc.termlist_end()) {
            ...
            LOGLINE(DB, "Try to find common prefix");
            LOGLINE(DB, "Term is: " << term << ", prev_term: " << prev_term);
            ...
            LOGLINE(DB, "New tag is: " << tag << ", wdf is: " << wdf);
