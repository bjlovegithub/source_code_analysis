...
open_stub(Database &db, const string &file)
{
    ...
    LOGLINE(DB, "open stub in dbfactory.cc, file name: " << file);
	...
	++line_no;
    LOGLINE(DB, file << " content: " << line); 
    LOGLINE(DB, "Create db in open_stub");
    ...
    LOGLINE(DB, "Return from open_stub");
}
